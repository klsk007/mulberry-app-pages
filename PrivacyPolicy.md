...
By default, AI answers use Apple's on-device Foundation Models framework when available. Users can explicitly select a local Llama 3.2 3B Instruct Q4_K_M model. The optional model is delivered on demand by the App Store, runs on device, and does not send prompts to a model download source.
...
Users can edit, export, or delete diaries in the app. Users can clear memory, restart the first-meeting flow, release the optional Offline Llama model for iPadOS storage reclamation, and revoke microphone or speech recognition permissions in iOS Settings.
